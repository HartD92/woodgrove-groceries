...
📌 Team update (2026-08-26T15:00:00-07:00): Issue #91 custom-domain default auth fix: root cause was not AFD query stripping and not `prompt=create` loss — live `/SignIn?handler=SignUpLink` still emitted `prompt=create`, and `infra/modules/frontDoor.bicep` has no rewrite/rules/query manipulation. The real gap was that issue #84 restored the default storefront authority to `hlacustomer.ciamlogin.com`, so only the explicit `CustomDomain` demo rewrote the browser-facing authorize host. I fixed this by keeping canonical MIW authority on the CIAM origin host, adding `AzureAd__CustomDomain`, centralizing redirect mutation in `AuthRedirectCustomizer`, and applying the branded host by default for all OIDC challenges while preserving prompt/login_hint/ui_locales/query-string parameters. Validation: `az bicep build` passed (existing BCP318 warnings only); live curl checks confirmed current deployed sign-up still carries `prompt=create` and custom-domain metadata resolves to `customers.hartlabs.info` authorize endpoints; `dotnet test` could not run locally because only SDK 9.0.317 is installed while the repo targets net10.0.

📌 Team update (2026-08-26T15:20:00-07:00): PR #93 follow-up: confirmed the remaining hostname leak was sign-out, not sign-in/sign-up. Live `curl` against `/MicrosoftIdentity/Account/SignOut` returned a 302 to `https://hlacustomer.ciamlogin.com/.../logout`, proving `OnRedirectToIdentityProviderForSignOut` was not yet using the custom-domain rewrite. I wired that sign-out event through `AuthRedirectCustomizer.Apply`, added logout-focused regression coverage, and updated the misleading infra/doc comments to reflect that AzureAd__CustomDomain is now default-on browser-facing behavior rather than demo-only config.
