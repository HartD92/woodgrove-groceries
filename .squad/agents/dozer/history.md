...
📌 Team update (2026-08-26T12:18:32-07:00): Investigated issue #84 (sign-up redirecting to login, suspected AFD). Found no AFD rewrite/cache/query-string rules in repo IaC; live `/SignIn?handler=SignUpLink` redirect still includes `prompt=create`. Most likely culprit is global deployment of `AzureAd__Authority`/`Instance` to the custom domain host in `infra/main.bicep`, which forces all auth flows through the custom-domain path instead of using CIAM origin by default. Recommended config review/fix; no code change made yet.

📌 Team update (2026-08-26T12:34:17-07:00): Implemented issue #84 fix on branch `squad/84-afd-signup-authority-fix`. Updated `infra/main.bicep` so default deployed `AzureAd__Authority`/`Instance` use `entraOriginHost` (CIAM origin) instead of the AFD custom domain; CustomDomain flow still overrides host through `Demos__CustomDomain`. Validated with `az bicep lint` + `az bicep build` (existing BCP318 warnings only).
📌 Team update (2026-08-26T13:12:38-07:00): Repaired `.github/workflows/squad-ci.yml` on branch `squad/fix-squad-ci-conflict-markers` by resolving committed merge-conflict markers in favor of the real .NET 10 restore/build/test workflow, while preserving useful push triggers for `dev` and `insider`. Validated the workflow parses as YAML after the fix.

📌 Team update (2026-08-26T13:26:38-07:00): Traced the repeated `Deploy Infrastructure` failures to the ExtID deployer bootstrap: `infra/README.md` documented `Policy.ReadWrite.AuthenticationMethod`, but the actual `az ad app permission add` commands never granted it to Identity B. Added the missing Microsoft Graph app-role assignment (`29c18626-4985-4dcd-85c0-193eef327366`) and explicit rerun/admin-consent guidance for existing deployer apps. Could not apply the live tenant grant from this environment because Azure CLI is not logged in, so a tenant admin still needs to run the documented grant for the current ExtID deployer app.
