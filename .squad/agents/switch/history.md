...
📌 Team update (2026-08-26T12:34:19-07:00): Implemented issue #83 frontend branding pass on squad/83-af-branding-frontend. Added src/storefront/wwwroot/css/af-theme.css, refreshed _Layout.cshtml, _LoginPartial.cshtml, and Pages/Index.cshtml for a moody A&F-inspired shell and pre-auth CTAs, produced matching Entra-hosted CSS assets at src/storefront/wwwroot/Company-branding/customcss.css and src/storefront/wwwroot/Company-branding/af-custom.css, and updated the layout to read any actual A&F logo/favicon from BrandAssets:BaseUrl with local Woodgrove fallbacks so no trademarked brand image bytes live in this public repo. Validation note: local dotnet build src/storefront/woodgrovedemo.csproj is blocked here because only .NET SDK 9.0.317 is installed while the project targets net10.0.

📌 Team update (2026-08-26T12:54:22-07:00): Addressed PR #86 review nit by making the layout favicon MIME type conditional: `image/png` when `BrandAssets:BaseUrl` supplies the external branded favicon and `image/x-icon` for the built-in Woodgrove fallback.

📌 Team update (2026-08-26T14:46:43-07:00): Reverted PR #86's storefront-only A&F shell for issue #90 on branch squad/90-revert-storefront-branding. Restored `Pages/Index.cshtml`, `Pages/Shared/_Layout.cshtml`, and `Pages/Shared/_LoginPartial.cshtml` to the pre-theme Woodgrove experience, removed `src/storefront/wwwroot/css/af-theme.css`, and intentionally left Entra Company Branding assets/docs/infra untouched so A&F remains only on the hosted sign-in/sign-up surfaces. Validation: `dotnet build src/storefront/woodgrovedemo.csproj --nologo` is still blocked locally by NETSDK1045 because this environment only has .NET SDK 9.0.317 while the project targets net10.0.
